...
export function OikosLeadForm() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const message = `Hola. Quiero coordinar un Diagnóstico Inmobiliario con Oikos & Argus. Inmobiliaria: ${formData.inmobiliaria}. Broker a cargo: ${formData.broker}. WhatsApp: ${formData.whatsapp}.`;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label className={labelClasses}>Nombre de la Inmobiliaria</label>
        <input
          type="text"
          required
          ...
          onChange={(e) =>
            setFormData({ ...formData, inmobiliaria: e.target.value })
          }
          className={inputClasses}
        />
      </div>
      <div>
        <label className={labelClasses}>Nombre del Broker a Cargo</label>
        <input
          type="text"
          required
      ...
      >
        Coordinar Diagnóstico Inmobiliario
      </button>
    </form>
  );
}
